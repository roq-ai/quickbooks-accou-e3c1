...
import { getJournals } from 'apiSdk/journals';
...
function DashboardPage() {
  ...
  const journalStat = useSWR<PaginatedInterface<JournalInterface>>(
    () => `/journals`,
    () => getJournals({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Journals',
      result: journalStat?.data?.totalCount,
      isLoading: journalStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Transactions',
